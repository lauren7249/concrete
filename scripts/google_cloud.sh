...
gcloud beta dataproc clusters create cluster-1 --zone us-east1-b --master-machine-type n1-standard-1 --master-boot-disk-size 2000 --num-workers 2 --worker-machine-type n1-standard-1 --worker-boot-disk-size 100 --project advisorconnect-1238 --initialization-actions 'gs://advisorconnect-jupyter/ipython.sh'


#create tunnel for jupyter
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ~/.ssh/google_compute_engine -N -f -L localhost:8888:localhost:8123 lauren@104.196.0.93
